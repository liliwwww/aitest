...
'''
需求：
我希望实现一个自动化的WEB控制器。通过程序模拟完成用户WEB操作。

比如，给定一个WEB界面，自动分析界面的元素，然后根据界面元素自动填写数据，并点击响应的执行按钮。
比如，在列表界面，可以输入各种查询条件， 然后点击查询，获得返回的列表信息。
在录入页面，根据控件的类型，自动的输入对应的数据。也需要支持select和日期选择框，然后点击执行按钮。并获取返回列表信息。

人类在进行页面操作时，需要大脑对页面Label和对应的控件进行判断。但是在WEB页面在定位控件时，因为缺少这一明确的对应关系，就需要借助其它的能力。
比如可以通过AI大模型解析html的相邻关系，另一方面，也希望能够借助于OpenCV实时监控Web界面的内容变化，然后输出Label标签值的变化以及页面信息的变化。

最终的结果，就是针对每一个页面的布局，生成一套高度可用的WEB界面控制指令。

最终，通过大模型技术，把人类输入的自然语言控制描述转换成WEB界面控制指令。

请你首先对我的整体需求进行理解，消化和吸收，然后结构化的复述一下我的需求
...
class DOMChangeMonitor:
                ...
                if (event.target.tagName === 'BUTTON') {
                    window.domChanges.push({
                        ...
                        target: {
                            ...
                            attributes: Array.from(event.target.attributes).reduce((obj, attr) => {
                                obj[attr.name] = attr.value;
                                return obj;
                            }, {}),
                            xpath: window.getXPath(event.target),
    ...
    def print_mutation(self, mutation):
        ...
        if mutation['type'] == 'attributes':
            ...
            log_message += f"  HTML: {outer_html}"
            logger.info(log_message)
        elif mutation['type'] == 'button_click':
            log_message = f"Button clicked at {xpath}\n"
            log_message += f"  Element: {target.get('nodeName', 'Unknown')}\n"
            log_message += f"  HTML: {outer_html}"
            logger.info(log_message)
        elif mutation['type'] == 'input_change':
            log_message = f"Input value changed at {xpath}\n"
            log_message += f"  Element: {target.get('nodeName', 'Unknown')}\n"
            log_message += f"  New value: {target.get('value', 'Unknown')}\n"
            log_message += f"  HTML: {outer_html}"
            logger.info(log_message)
        elif mutation['type'] == 'select_change':
            log_message = f"Select value changed at {xpath}\n"
            log_message += f"  Element: {target.get('nodeName', 'Unknown')}\n"
            log_message += f"  New value: {target.get('value', 'Unknown')}\n"
            log_message += f"  HTML: {outer_html}"
            logger.info(log_message)
