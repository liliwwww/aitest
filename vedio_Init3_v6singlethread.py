...
class CameraApp:
    ...
    def touchAutoClick( self,frame, current_region, output_dir):
        ...
        self._save_screenshots(frame, current_region, output_dir, timestamp)
    ...
    def camera_capture_thread(self, output_dir="screenshots"):
        ...
        while self.running and self.cap.isOpened():
            ...
            if countA % 1000 == 0 :
               print(f" camera_capture_thread read frame size {countA} queue size {self.frame_queue.qsize()}")
            ...
            if self.prev_frame is not None:
                ...
                if isChange:
                    ...
                    if self._touchAutoClick_ :
                        #self.touchAutoClick1(frame,current_region,output_dir)
            ...
            try:
                ...
                if self.frame_queue.full():
                    print(f"如果队列满了，先移除最旧的帧{self.frame_queue.qsize()}")
    ...
    def on_mouse_drag(self, event):
        """鼠标拖拽事件处理"""
        #print("here **")
        if not self.running or not (self.dragging or self.resizing or self.dragging_retry or self.resizing_retry):
            print("here ??")
            return
