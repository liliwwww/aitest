...
def compare_dom_nodes(node1, node2):
    ...
    if len(node1.contents) != len(node2.contents):
        return False
    for child1, child2 in zip(node1.contents, node2.contents):
        if not compare_dom_nodes(child1, child2):
            return False
    return True

def find_different_nodes(node1, node2):
    different_nodes = []
    if not compare_dom_nodes(node1, node2):
        different_nodes.append((node1, node2))
    for child1, child2 in zip(node1.contents, node2.contents):
        different_nodes.extend(find_different_nodes(child1, child2))
...
def compare_html_files(file1_path, file2_path):
    try:
        ...
        different_nodes = find_different_nodes(soup1, soup2)
        for node1, node2 in different_nodes:
