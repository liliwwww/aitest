...
class CameraApp:
    ...
    def create_widgets(self):
        ...
        self.record_button = ttk.Button(row1_frame, text="开始录制", command=self.save_Every_Frame)
        self.record_button.pack(side=tk.LEFT, padx=5)
    ...
    def _save_frames_worker(self):
        ...
        while True:
            try:
                ...
                if( self.save_queue_interval % 100 == 0 ):
                    print(f"帧保存线程已处理帧数: {self.save_queue_interval}")
                    # 生成文件名（使用时间戳确保唯一性）
                    
                    filename = os.path.join("metric", f"Metric_v8_{timestamp}.jpg")                
                    # 保存帧到文件
        ...
        while self.runningCamera:
            try:
                ...
                if self.save_Every_Frame_Running:
                    # 将帧放入保存队列                    
                    self.save_queue.put((frame.copy(), time.time()))  # 使用copy防止引用问题
