...
from vedio_type2_v2 import do_task
...
class CameraApp:
    def __init__(self, root):
        ...
        # 创建帧保存队列和处理线程
        self.save_Every_Frame_Running = False
        self.save_queue_interval = 0
        self.save_queue_interval_threshold = 1

        self.save_queue = queue.Queue()  # 设置最大队列长度，防止内存溢出
        self.save_thread = threading.Thread(target=self._save_frames_worker, daemon=True)
        ...
        self.gui_display_flag = True

        self.avg_cpu_usage = 0.0  # 存储最近的 CPU 使用率
        ...
        #进程管理
        self.timer_process = None  # 存储子进程对象

        self.process_communication_queue = multiprocessing.Queue()  # 进程间通信队列（可选）

        self.process_queue_thread = threading.Thread(target=self.read_queue_cross_process, daemon=True)
        self.process_queue_thread.start()
        print("进程间通信队列线程已启动")

    def read_queue_cross_process(self):
        """在独立线程中读取队列消息"""
        while True:
            try:
                # 非阻塞读取队列，超时时间0.5秒
                timestampIn = self.process_communication_queue.get(timeout=0.5)
                timestampDiff = time.time()*1000 - timestampIn
                # 告诉esp32，click

                print("\n 转化队列  FROM process_communication_queue TO  task_queue \n")
                # 将任务放入队列，由后台线程处理
                self.task_queue.put((self.touchAutoClick, (timestampIn, timestampDiff)))
                
                # 标记任务完成
                self.process_communication_queue.task_done()
            except queue.Empty:
                # 队列为空时继续循环
                continue
    ...
    def _on_start_timer(self):
        ...
        try:
            adjust_time = int(self.adjust_time_entry.get())
            self.startup_timer_process(target_time, adjust_time)
        except ValueError:
            messagebox.showerror("输入错误", "调整毫秒数必须为整数")


    def startup_timer_process(self, target_time_str, adjust_time=0):
        """启动独立进程运行定时任务"""
        # 停止已有进程（避免重复启动）
        ...
        self.timer_process = multiprocessing.Process(
            target=do_task,  # 子进程入口（需从 vedio_type2_v2 导入）
            args=(target_time_str, adjust_time, self.process_communication_queue),  # 传递目标时间和调整毫秒数
            daemon=True  # 主程序退出时自动终止子进程
        )
        
        # 启动子进程
        self.timer_process.start()
        print(f"独立定时任务进程已启动，PID: {self.timer_process.pid}")

    def shutdown_timer_process(self):
        """停止定时任务进程"""
        if self.timer_process and self.timer_process.is_alive():
            self.timer_process.terminate()  # 强制终止
            self.timer_process.join(timeout=2)  # 等待退出
            print("定时任务进程已停止")
    ...
    def update_Maxtrixtime(self):
        """更新时间显示，并设置下一次更新"""
        # 获取当前系统时间（精确到毫秒）
        current_time = time.time()
        formatted_time = f"当前时间: {current_time:.3f} 秒 (Unix时间戳)"
        
        # 更新变量（会自动同步到标签）
        self.change_var.set(formatted_time)

        # 设置1毫秒后再次调用更新方法（1000毫秒=1秒）
        self.root.after(20, self.update_Maxtrixtime)
    ...
    def create_widgets(self):
        ...
        self.record_button = ttk.Button(row1_frame, text="关闭GUI", command=self.close_GUI)
        self.record_button.pack(side=tk.LEFT, padx=5)

        self.record_button = ttk.Button(row1_frame, text=f"开始录制{self.save_queue_interval_threshold}", command=self.save_Every_Frame)
        self.record_button.pack(side=tk.LEFT, padx=5)
        ...
        # 定时任务控制区
        timer_frame = ttk.LabelFrame(control_frame, text="定时任务控制")
        timer_frame.grid(row=2, column=0, columnspan=2, sticky="ew", padx=5, pady=5)
        
        # 目标时间输入
        ttk.Label(timer_frame, text="目标时间 (HH:MM:SS.fff):").pack(side=tk.LEFT, padx=5)
        self.target_time_entry = ttk.Entry(timer_frame, width=15)
        self.target_time_entry.insert(0, "19:17:00.123")  # 默认值
        self.target_time_entry.pack(side=tk.LEFT, padx=5)
        
        # 调整时间输入
        ttk.Label(timer_frame, text="调整毫秒数:").pack(side=tk.LEFT, padx=5)
        self.adjust_time_entry = ttk.Entry(timer_frame, width=6)
        self.adjust_time_entry.insert(0, "123")  # 默认调整 123ms
        self.adjust_time_entry.pack(side=tk.LEFT, padx=5)
        
        # 启动/停止按钮
        ttk.Button(timer_frame, text="启动定时任务", command=self._on_start_timer).pack(side=tk.LEFT, padx=5)
        ttk.Button(timer_frame, text="停止定时任务", command=self.shutdown_timer_process).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(cmd_frame, text="back", command=lambda: self.send_command("back99")).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(cmd_frame, text="CPU_STAT", command=self.send_thread_command).pack(side=tk.LEFT, padx=5)
        ...
        self.display_frame.pack(fill=tk.BOTH, padx=10, pady=5)
        ...
        # 创建字体对象
        my_font = font.Font(family="SimHei", size=40)  # 设置字体和初始大小
        ...
        self.change_label.pack(fill=tk.BOTH, expand=True, padx=5, pady=2)  # 填满整个框架

        self.update_Maxtrixtime()
    ...
    def start_cpu_monitor(self):
        ...
        def monitor_cpu():
            while self.cpu_monitor_running:
                self.avg_cpu_usage = self.get_process_cpu_usage(10)
                # 每 10 秒更新一次 CPU 使用率
                time.sleep(10)  # 短暂休眠以降低线程开销
    ...
    def _print_frame_statistics(self):
        ...
        msg = f"[    Average CPU usage over 10 seconds: {self.avg_cpu_usage:.2f}%"
        print(msg)

        #一次点击状态
        print(f"[    一次点击状态 {self.oneclickRest_flag}   __是否完成-> {self.oneclickRest_hasDone}")
        ...
        print(f"[    当前 CPU 使用率: {self.avg_cpu_usage:.2f}%")
    ...
    # 通用处理
    def common_process(self, type, *param):
        
        #print(f"common_process: {type}, {param}    {self.common_queue.qsize()}  \n\n")
        
        #bbb = time.time()
        #上传日志
        if type == "upload_log":
            self.client.upload_log(*param)

        #保存截图
        if type == "_save_screenshots":
            self._save_screenshots(*param)

        # 添加性能统计
        if type == "add_sample":
            self.performance_stats.add_sample(*param)
            interval1, interval2, interval3, interval4 = param
            print(f"从读取到frame到触发 耗时step1检测出差异({interval1}) step3执行点击完成({interval3}) step4总耗时({interval4})毫秒")  

        #ccc = time.time()
        #print(f"common_process {type} 耗时: {ccc - bbb:.2f}秒")

    # 触发变化的时候，发送串口，并截屏
    # 1点击， 2截屏，3并截取区域 4.保存目录    
    def touchAutoClick( self, timestampIn, timestampDiff):
        ...
        try:           
            ...
            if not self.oneclickRest_hasDone:                                        
                ...
                autoClickCmd = f"back{random_number}"        
                #print(f"命令AUTO: cmd->{autoClickCmd}  num->{self.click23_Num}")
                
                response = self.ledController.control_led( autoClickCmd, self.click23_Num)
                #点击完成
                timeStampClickOver = time.time()*1000
                print(f"响应AUTO->:cmd->{autoClickCmd}  num->{self.click23_Num} reponse-> {response}")
                ...
                # 上传日志
                self.common_queue.put((self.common_process, ( "upload_log", f"STEP3 CLICK---@{time.time()*1000}" )))
                
                
                # 记录时间间隔数据
                interval1 = timestampDiff - timestampIn
                interval3 = timeStampClickOver - timestampDiff
                interval4 = timeStampClickOver - timestampIn
                self.common_queue.put((self.common_process, ( "add_sample", interval1, 0, interval3, interval4 )))        
        
        except Exception as e:            
            print(f"发送命令<touchAutoClick>错误: {e}")
            traceback.print_exc()  
    ...
    def _process_common_tasks(self):
        while True:
            try:
                # 获取任务（带超时，避免永久阻塞）
                task = self.common_queue.get(timeout=1)
                if task is None:  # 退出信号
                    continue
                #print(f"_process_common_tasks 拿到任务了...{task}")
                # 执行任务
                func, args = task
                func(*args)
                self.common_queue.task_done()
                #print(f"_process_common_tasks 任务完成了...{task}")
            except queue.Empty:
                continue
    ...
    def _calculateDiff(self, frameFull, timestampIn, output_dir ):
        ...
        if self.prev_frame is not None:
            ...
            if self._touchAutoClick_ and isChange:
                ...
                timestampDiff = time.time()*1000

                
                self.lastChangeDetected = timestampDiff
                ...
                # 将辅助任务放入队列，由后台线程处理
                self.common_queue.put((self.common_process, ( "upload_log", f"STEP2 _DIFF---@{time.time()*1000}" )))
                self.common_queue.put((self.common_process, ( "_save_screenshots", frameFull, current_region, timestampIn, output_dir )))
    ...
    def update_gui(self):
        #print("更新 GUI........")
        if self.gui_display_flag:
            with self.frame_lock:
                if self.latest_frame is not None:
                    self._update_frame_draw(self.latest_frame)
            self.root.after(200, self.update_gui)  # Update every 100ms
            
    
    
    #为了精确控制， 需要记录每一帧的信息
    def _save_frames_worker(self):
        """帧保存工作线程"""
        #print("帧保存线程已启动")
        ...
        while True:
            try:
                # 获取要保存的帧和时间戳
                frame, timestamp = self.save_queue.get(timeout=1)
                
                self.save_queue_interval +=1
                if( self.save_queue_interval % self.save_queue_interval_threshold == 0 ):
                    print(f"帧保存线程已处理帧数: {self.save_queue_interval}")
                    # 生成文件名（使用时间戳确保唯一性）
                    
                    self.fileSeq = self.fileSeq +1
                    filename = os.path.join("metric", f"MetricCluster_v8_{timestamp}_{self.fileSeq}.jpg")
                    # 保存帧到文件
                    """将帧保存到文件的实际方法，可根据需要修改"""
                    # 这里使用OpenCV保存图像，你也可以使用PIL或其他库
                    try:
                        cv2.imwrite(filename, frame)
                        print(f"保存帧到文件: {filename} 待处理队列长度{self.save_queue.qsize()}")
                    except Exception as e:
    ...
    def retrieve_frames(self):
        ...
        while self.runningCamera:
            try:
                # step 1/3 从grab队列获取任务
                frame, timestampIn = self.grab_queue.get(timeout=1)

                # step 2/3 处理图片差异；
                self._calculateDiff(frame, timestampIn, self.output_dir)

                # step 3/3 Update latest_frame for GUI
                copyFrame = frame.copy()
                with self.frame_lock:
                    self.latest_frame = copyFrame

                if self.save_Every_Frame_Running:
                    # 将帧放入保存队列
                    self.save_queue.put((copyFrame, timestampIn))  # 使用copy防止引用问题
    ...
    def start_monitoring(self):
        ...
        self.ESP32_thread = threading.Thread(target=self._process_touch_tasks, daemon=True)
        self.ESP32_thread.start()
        print("启动单片机线程")

        self.common_queue = queue.Queue()
        self.common_thread = threading.Thread(target=self._process_common_tasks, daemon=True)
        self.common_thread.start()
        print("启动通用处理线程")
    ...
    def stop_monitoring(self):
        ...
        self.ESP32_thread.join(timeout=2)  # 等待线程结束
        print("停止单片机线程")
    ...
    def send_custom_command(self):
        ...
        try:
            num = int(cmd)  # 尝试转换为整数
            # 生成一个范围在 11 到 98 之间（包含 11 和 98）的随机整数
            random_number = 999
    ...
    def get_process_cpu_usage(self, duration=10):
        """
        线程名称: MainThread, 线程 ID: 39220
        线程名称: Thread-1 (_save_frames_worker), 线程 ID: 33540        保存每一帧
        线程名称: Thread-2 (monitor_cpu), 线程 ID: 37612                统计线程
        线程名称: Thread-3 (camera_capture_thread), 线程 ID: 35620      摄像头捕获线程
        线程名称: Thread-4 (retrieve_frames), 线程 ID: 31916            帧处理线程
        线程名称: Thread-5 (_process_touch_tasks), 线程 ID: 18772       触摸任务处理线程(1)发给ESP32指令
        线程名称: Thread-6 (_process_common_tasks), 线程 ID: 38092      通用任务处理线程(1)upload_log(2)save_screenshoot(3)统计耗时()()
        """
        # 获取当前进程的 PID
    ...
    def close_GUI(self):
        ...
        self.gui_display_flag = not self.gui_display_flag
        ...
        print(f"_close_GUI {self.gui_display_flag}")

    def save_Every_Frame(self):

        self.save_Every_Frame_Running = not self.save_Every_Frame_Running

        if self.save_Every_Frame_Running:
            self.record_button.config(text="停止录制")
        else:
            self.record_button.config(text=f"开始录制{self.save_queue_interval_threshold}")
        print(f"_录制状态: {self.save_Every_Frame_Running}")
