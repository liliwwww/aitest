...
class CameraApp:
    ...
    def touchAutoClick( self, frame, current_region,timestampIn, output_dir):
        ...
        print(f"从读取到frame到触发， 耗时{time.time()*1000 - timestampIn}毫秒")
            ...
            if self.prev_frame is not None:
                ...
                if isChange:
                    ...
                    if self._touchAutoClick_ :
                        ...
                        self.touchAutoClick(frameFull,current_region,timestampIn, output_dir)
        ...
        while self.runningCamera:
            try:
                ...
                if _retrieve_frames_count % 1000 == 0:
                    print(f"retrieve帧数 ->1: {self.grab_queue.qsize()}  , ->2 {self.frame_queue.qsize()}")
                ...
                # 放到 frame_queue中
                # 供显示队列使用
                self.frame_queue.put((frame, timestampIn))     
