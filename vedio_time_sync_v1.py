    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            for i in range(TOTAL_ROUNDS):
                ...
                # 发送客户端时间到服务器（使用pickle序列化long类型）
                data = pickle.dumps(client_time)
                s.sendall(data)
                logging.debug(f"第{i+1}次通信: 发送客户端时间 {client_time}")
                
                # 接收服务器时间（8字节）
                server_time_data = receive_full_data(s, 2)
                if not server_time_data:
                    logging.error("接收服务器时间失败，数据不完整")
                    return "接收服务器时间失败：数据不完整"
                
                server_time = pickle.loads(server_time_data)
        ...
        return f"数据解析错误：{e}，请检查服务器返回的数据格式"
